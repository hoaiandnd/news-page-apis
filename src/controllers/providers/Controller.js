...
class Controller {
  ...
  static extendedResponseMethods(res) {
    ...
    return {
      ...
      internalServerError: (res, json) => {
        const {
          message = responseMessage.fail.internalServerError,
          data = null,
          status = 500
        } = json
        res.status(status).json({ message, data })
      ...
      badRequest: (res, json) => {
        const {
          message = responseMessage.fail.badRequest,
          data = null,
          status = 400
        } = json
        res.status(status).json({ message, data })
   ...
   * * Combine the `request`, `response`, `service`, and `next` parameters into a single object.
   * * Inject response methods into the `response` object.
   * @param {string} actionName action function return a promise
   * @param {any} service service object. Can be any object will be passed to action function with `service` key
